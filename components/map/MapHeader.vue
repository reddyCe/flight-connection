  ...
  <div class="absolute top-0 left-0 right-0 z-[1000] p-2 md:p-4 pointer-events-none">
      <div class="flex flex-col gap-2 md:gap-4 pointer-events-auto">
        ...
        <div class="flex items-start justify-between gap-2 md:gap-4">
          ...
          <!-- Branding / Main Stats -->
          <div v-if="sequence.length === 0" class="map-card px-3 py-2 md:px-4 md:py-3 flex items-center gap-2 md:gap-4">
          ...
          <div v-else class="map-card p-1.5 md:p-2 w-auto max-w-[calc(100vw-5.5rem)] md:max-w-[90vw] overflow-hidden flex flex-col gap-1.5 md:gap-2 transition-all duration-300 ease-in-out">
             <div class="flex items-center justify-between px-1.5 md:px-2 pt-0.5 md:pt-1 gap-2 md:gap-6">
                <div class="flex items-center gap-2 md:gap-4">
                  <span class="text-[9px] md:text-[10px] font-black uppercase text-muted-foreground flex items-center gap-1 md:gap-1.5 tracking-widest">
                    <Plane class="w-3 h-3 md:w-3.5 md:h-3.5 text-primary" />
                    <span class="hidden sm:inline">Flight Plan</span>
                  </span>

                  <Popover>
                    <PopoverTrigger as-child>
                      <button class="flex items-center gap-1 md:gap-2 px-1.5 md:px-2 py-1 bg-muted/50 rounded-md border border-border/50 hover:bg-muted transition-colors outline-none focus:ring-2 focus:ring-primary/20">
                        <CalendarIcon class="w-3 h-3 text-muted-foreground" />
                        <span class="text-[9px] font-bold font-mono">{{ startDate }}</span>
                      </button>
                    </PopoverTrigger>
                ...
                <div class="flex items-center gap-1 md:gap-2">
                  <Popover>
                    <PopoverTrigger as-child>
                       <Button
                        variant="ghost"
                        size="xs"
                        class="h-7 text-[9px] md:text-[10px] font-bold uppercase hover:text-primary gap-1 px-1.5 md:px-2"
                      >
                        <History class="w-3 h-3" />
                        <span class="hidden sm:inline">Saved</span>
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent class="w-56 md:w-64 p-0" align="end">
                      <div class="p-2 border-b border-border bg-muted/30">
                        <span class="text-xs font-bold uppercase">Saved Routes</span>
                      </div>
                      <div v-if="savedRoutes.length === 0" class="p-4 text-center text-xs text-muted-foreground">
                  ...
                  <Button
                    variant="ghost"
                    size="xs"
                    class="h-7 text-[9px] md:text-[10px] font-bold uppercase hover:text-destructive gap-1 px-1.5 md:px-2"
             ...
             <!-- Breadcrumbs -->
             <div class="flex items-center gap-1 md:gap-2 overflow-x-auto pb-1.5 md:pb-2 px-1.5 md:px-2 mask-linear">
                <div v-for="(stop, idx) in sequence" :key="stop.iata_code" class="flex items-center shrink-0">
                   <Badge
                      ...
                      class="font-mono text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 md:py-1 gap-1 md:gap-2"
                   >
                      {{ stop.iata_code }}
                      <span v-if="idx === 0" class="w-1.5 h-1.5 rounded-full bg-green-400"></span>
                      <span v-if="idx === sequence.length - 1 && idx > 0" class="w-1.5 h-1.5 rounded-full bg-amber-400"></span>
                   </Badge>
                   <ArrowRight v-if="idx < sequence.length - 1" class="w-2.5 h-2.5 md:w-3 md:h-3 text-muted-foreground mx-0.5 md:mx-1" />
                </div>
                <!-- Prompt for next -->
                <div class="flex items-center shrink-0 opacity-50 animate-pulse">
                   <ArrowRight class="w-2.5 h-2.5 md:w-3 md:h-3 text-muted-foreground mx-0.5 md:mx-1" />
                   <div class="border border-dashed border-muted-foreground/50 rounded px-1.5 md:px-2 py-0.5 md:py-1 text-[9px] md:text-[10px] uppercase font-bold text-muted-foreground">
                      <span class="hidden sm:inline">Select Destination</span>
